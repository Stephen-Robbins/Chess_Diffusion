...
def print_chessboard_from_fen(fen):
    board = chess.Board(fen)  # Append a standard ending for completeness
...
def fen_to_bitboard(fen):
    ...
    return(torch.tensor(board_array))
...
def bitboard_to_fen(bitboard):
    ...
    if isinstance(bitboard, torch.Tensor):
        bitboard = bitboard.numpy()
...
def process_tensor(tensor):
    ...
    # Step 2: Set the maximum positive value to 1 and others to 0
    for i in range(tensor.shape[1]):  # Iterate through 8x8 matrices
        for j in range(tensor.shape[2]):
            # Flatten the (12, ) vector, find the index of the max value
            max_val_index = torch.argmax(tensor[:, i, j])
            max_val = tensor[max_val_index, i, j]

            if max_val > 0:  # Check if the maximum value is positive
                # Set all values to 0
                tensor[:, i, j] = 0
                # Set only the maximum value to 1
                tensor[max_val_index, i, j] = 1
...
class ChessDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tensor = fen_to_bitboard(fen) 
        transformed_tensor = 2 *  torch.tensor(tensor, dtype=torch.float32) - 1
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            transformed_tensor = process_tensor(sample_tensor[0].squeeze())
                            fen = bitboard_to_fen(transformed_tensor)
